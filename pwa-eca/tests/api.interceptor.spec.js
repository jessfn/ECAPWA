// pwa-eca — pruebas del interceptor de refresh en 401 (ECA-011).
// A diferencia de admin-eca, un refresh fallido aquí cierra solo la sesión
// de *servidor* (`cerrarSesionDeServidor`), no la sesión local offline.
...
describe('manejarErrorDeRespuesta', () => {
  ...
  it('si el refresh también falla, cierra solo la sesión de servidor (no la local)', async () => {
    ...
    expect(authFalso.cerrarSesionDeServidor).toHaveBeenCalledTimes(1)
